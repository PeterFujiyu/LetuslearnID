...
首个注册用户将自动加入 `admin` 用户组。管理员可在 `/manage` 页面进入后台并访问 `/admin/` 界面。该页面现在提供完整的用户和用户组管理功能，并新增“SQL CLI”窗口可直接执行 SQL 语句查看结果。管理员可修改任意用户的密码、两步验证状态、通行密钥绑定以及所属组，并支持在线新增或编辑用户组配置。页面还能查看指定用户的验证码，如不存在会自动生成。
